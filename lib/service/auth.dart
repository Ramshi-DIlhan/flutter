...
import 'package:gitpod_flutter_quickstart/service/storage.dart';

final storage = StorageService();

class AuthService {
  final _auth = FirebaseAuth.instance;
  Future<String> SignUp(String name, String dept, String email, String password,
      String phone) async {
    try {
      final result = await _auth.createUserWithEmailAndPassword(
          email: email.trim(), password: password);
      final image = await camcontroller.takePicture();
      await storage.uploadFile(image.path, result.user!.email);

      DatabaseService(uid: result.user!.uid)
          .initPlayer(name, dept, email.trim(), phone);
      Get.off(() => QrScanPage(
            uid: result.user!.uid,
          ));
      return '';
      ...
      if (e.code == 'wrong-password')
        ...
        return 'Email Format is not correct (eg:example@gmail.com)';
